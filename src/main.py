...
def get_keywords2(text, nlp):
    ...
    sorted_items = sorted(rake_nltk_var.get_word_degrees().items(), \
...
def clean_res():
    for article, i in retdict.items():
        retdict[article] = {elem[0]: elem[1] \
            for elem in sorted(i.items(), key=lambda x: x[1]["score"], reverse=True)[:3]}
...
def main():
    ...
    df_articles = pd.read_json('articles.json', orient='index')
    df_videos = pd.read_json('videos.json', orient='index')
    ...
    for i, elem in enumerate(df_articles['text']):
        ...
        for j, elem2 in enumerate(df_videos['text']):
            ...
            retdict[df_articles['text'].index[i]][df_videos['text'].index[j]]["score"] \
                = 0.4 * comp_val + 0.3 * comp_val2 + 0.3 * comp_val3
    clean_res()
